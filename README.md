...
7. Create a Firestore database and use rules that restrict `users/{userId}` to `request.auth.uid == userId` (example in comments below).

### Example Firestore rules (starter)

Profile doc and transaction subcollection `users/{uid}/transactions/{txId}`:
